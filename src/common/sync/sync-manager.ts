...
export async function syncTimeline(
  ...
  try {
    ...
    const sinceTimestamp = newestTimestamp > 0 ? newestTimestamp : Math.floor(Date.now() / 1000) - 3600; // Last hour as fallback

    console.log(`[SyncManager] Syncing ${type} timeline since ${new Date(sinceTimestamp * 1000).toISOString()}`);

    // Fetch new events from relays
    const relays = getRelays();
    const newEvents = await fetchNewEvents(type, relays, sinceTimestamp, limit, followedPubkeys);
    ...
    console.log(`[SyncManager] Synced ${newEvents.length} new events for ${type} timeline`);
      ...
      await new Promise<void>((resolve) => {
        ...
        socket.onmessage = (msg: MessageEvent): void => {
          try {
            ...
            console.warn(`[SyncManager] Failed to parse message from ${relayUrl}:`, e);
          }
        };
 ...
 */
export async function syncGlobalTimeline(options?: SyncOptions): Promise<SyncResult> {
