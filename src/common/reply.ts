...
export function setupReplyOverlay(options: ReplyOverlayOptions): void {
  ...
  const backdrop: HTMLElement | null = document.getElementById("reply-overlay-backdrop");
  const closeBtn: HTMLElement | null = document.getElementById("reply-overlay-close");
  const textarea: HTMLTextAreaElement | null = document.getElementById("reply-textarea") as HTMLTextAreaElement;
  const submitBtn: HTMLButtonElement | null = document.getElementById("reply-submit") as HTMLButtonElement;
  const statusEl: HTMLElement | null = document.getElementById("reply-status");
  const originalPostEl: HTMLElement | null = document.getElementById("reply-original-post");

  if (!overlay || !backdrop || !closeBtn || !textarea || !submitBtn || !statusEl || !originalPostEl) {
    return;
  }
  ...
  const openOverlay = (context: ReplyContext): void => {
    ...
    const truncatedContent = context.eventContent.length > 200
  ...
  submitBtn.addEventListener("click", async (): Promise<void> => {
    ...
    try {
      const hasExtension: boolean = Boolean((window as any).nostr && (window as any).nostr.signEvent);
      const sessionPrivateKey: Uint8Array | null = options.getSessionPrivateKey();
